...
def register_omnilingual_asr_wav2vec2_asr_configs(
    ...
    @arch("1b_514", advanced=True)
    def _1b_asr_514(resolver: DependencyResolver) -> Wav2Vec2AsrConfig:
        # 1B 512-vocab variants extended with the KSOF disfluency markers
        # <uf> (id 512) and <m> (id 513).
        config = get_config(resolver, Wav2Vec2AsrConfig, "base_10h")
        config.encoder_config = get_config(
            resolver, Wav2Vec2Config, "1b"
    ...
    def _300m_asr_v2_ufm(resolver: DependencyResolver) -> Wav2Vec2AsrConfig:
        # omniASR written-v2 vocab extended with the KSOF disfluency markers
        # <uf> (id 10288) and <m> (id 10289).
        config = get_config(resolver, Wav2Vec2AsrConfig, "base_10h")
        config.encoder_config = get_config(
            resolver, Wav2Vec2Config, "large_lv60k"
        ).encoder_config
        ...
        config.max_temporal_mask_prob = 0.0
        config.max_spatial_mask_prob = 0.0
        config.target_vocab_size = 10290
    ...
    def _1b_asr_v2_ufm(resolver: DependencyResolver) -> Wav2Vec2AsrConfig:
        # 1B omniASR written-v2 vocab extended with the KSOF disfluency
        # markers <uf> (id 10288) and <m> (id 10289).
        config = get_config(resolver, Wav2Vec2AsrConfig, "base_10h")
        config.encoder_config = get_config(
            resolver, Wav2Vec2Config, "1b"
        ).encoder_config
